...
(function () {
    ...
    class GameSimulation extends Simulation {
        ...
        generateFood() {
            const gridRes = 70;
            let [startX, startY, ] = this.topLeftCorner;
            let [lengthX, lengthY, ] = vec.distance(this.topLeftCorner, this.bottomRightCorner);
            ...
            for (let yc = 0; yc < gridRes; ++yc) {
                for (let xc = 0; xc < gridRes; ++xc) {
                    ...
                    if (v > threshold) {
                        let particle = this.spawn(particleType.FOOD);
